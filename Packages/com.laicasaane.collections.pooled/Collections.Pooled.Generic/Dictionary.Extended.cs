...
{
    partial class Dictionary<TKey, TValue> : IDisposable
    {
        public Dictionary((TKey Key, TValue Value)[] array, IEqualityComparer<TKey>? comparer)
            : this(array.AsSpan(), comparer, ArrayPool<int>.Shared, ArrayPool<Entry<TKey, TValue>>.Shared)
        { }

        public Dictionary((TKey Key, TValue Value)[] array, IEqualityComparer<TKey>? comparer, ArrayPool<int> bucketPool, ArrayPool<Entry<TKey, TValue>> entryPool)
            : this(array.AsSpan(), comparer, bucketPool, entryPool)
        { }

        public Dictionary(KeyValuePair<TKey, TValue>[] array, IEqualityComparer<TKey>? comparer)
            : this(array.AsSpan(), comparer, ArrayPool<int>.Shared, ArrayPool<Entry<TKey, TValue>>.Shared)
        { }

        public Dictionary(KeyValuePair<TKey, TValue>[] array, IEqualityComparer<TKey>? comparer, ArrayPool<int> bucketPool, ArrayPool<Entry<TKey, TValue>> entryPool)
            : this(array.AsSpan(), comparer, bucketPool, entryPool)
        { }

        public Dictionary(KVPair<TKey, TValue>[] array, IEqualityComparer<TKey>? comparer)
            : this(array.AsSpan(), comparer, ArrayPool<int>.Shared, ArrayPool<Entry<TKey, TValue>>.Shared)
        { }

        public Dictionary(KVPair<TKey, TValue>[] array, IEqualityComparer<TKey>? comparer, ArrayPool<int> bucketPool, ArrayPool<Entry<TKey, TValue>> entryPool)
            : this(array.AsSpan(), comparer, bucketPool, entryPool)
        { }

        public Dictionary(in ReadOnlySpan<(TKey Key, TValue Value)> span, IEqualityComparer<TKey>? comparer, ArrayPool<int> bucketPool, ArrayPool<Entry<TKey, TValue>> entryPool)
        ...
        public Dictionary(in ReadOnlySpan<KeyValuePair<TKey, TValue>> span, IEqualityComparer<TKey>? comparer, ArrayPool<int> bucketPool, ArrayPool<Entry<TKey, TValue>> entryPool)
            : this(span.Length, comparer, bucketPool, entryPool)
        {
            foreach (var pair in span)
            {
        ...
        public Dictionary(in ReadOnlySpan<KVPair<TKey, TValue>> span, IEqualityComparer<TKey>? comparer, ArrayPool<int> bucketPool, ArrayPool<Entry<TKey, TValue>> entryPool)
            : this(span.Length, comparer, bucketPool, entryPool)
        {
            foreach (var pair in span)
            {
        ...
        public void CopyTo(in Span<KeyValuePair<TKey, TValue>> dest, int destIndex, int count)
        {
            ...
            Span<Entry<TKey, TValue>> src = _entries.AsSpan(0, _count);
