...
namespace ZBase.Collections.Pooled.Generic.Extensions
{
    public static partial class ListExtensions
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Extensions<T> GetExtensions<T>(this ZCPG.List<T> self)
            => new Extensions<T>(self);

        public readonly ref struct Extensions<T>
        {
            private readonly ZCPG.List<T> _list;

            internal Extensions(ZCPG.List<T> list)
            {
                _list = list;
            }
            ...
            public ZCPG.List<T> FindAll<TPredicate>(TPredicate match) where TPredicate : IPredicate<T>
            {
                if (match == null)
                {
                    ThrowHelper.ThrowArgumentNullException(ExceptionArgument.match);
                ...
                ZCPG.List<T> list = new ZCPG.List<T>();
                T[] items = _list._items;
