...
{
    public static partial class TempListExtensions
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Extensions<T> GetExtensions<T>(in this ZCPG.TempList<T> self)
            => new Extensions<T>(self);

        public ref struct Extensions<T>
        {
            private TempList<T> _list;

            internal Extensions(TempList<T> list)
            {
                _list = list;
            }
            ...
            public ZCPG.TempList<TOut> ConvertAll<TOut, TConverter>(TConverter converter) where TConverter : IConverter<T, TOut>
            {
                if (converter == null)
                {
                    ThrowHelper.ThrowArgumentNullException(ExceptionArgument.converter);
                ...
                ZCPG.TempList<TOut> list = ZCPG.TempList<TOut>.Create(_list._size);
                T[] src = _list._items;
            ...
            public ZCPG.TempList<T> FindAll<TPredicate>(TPredicate match) where TPredicate : IPredicate<T>
            {
                if (match == null)
                {
                    ThrowHelper.ThrowArgumentNullException(ExceptionArgument.match);
                ...
                ZCPG.TempList<T> list = new ZCPG.TempList<T>();
                T[] items = _list._items;
