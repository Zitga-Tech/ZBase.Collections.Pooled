...
namespace ZBase.Collections.Pooled.Generic
{
    ...
    /// </remarks>
    [Serializable]
    public ref partial struct TempArrayHashSet<T>
        where T : notnull
    {
        ...
        public static TempArrayHashSet<T> Create()
            => new TempArrayHashSet<T>(0
        ...
        public static TempArrayHashSet<T> Create(int capacity)
            => new TempArrayHashSet<T>(capacity
                , ArrayPool<ArrayEntry<T>>.Shared
                , ArrayPool<int>.Shared
            );
        ...
        public static TempArrayHashSet<T> Create(int capacity
            ...
            => new TempArrayHashSet<T>(capacity, entryPool, bucketPool);

        internal TempArrayHashSet(int capacity
            , ArrayPool<ArrayEntry<T>> entryPool
            , ArrayPool<int> bucketPool
        )
            : this()
        ...
        public ref struct Enumerator
        {
            ...
            private readonly TempArrayHashSet<T> _set;
            ...
            public Enumerator(in TempArrayHashSet<T> set)
            {
                _set = set;
                _index = -1;
                _count = set.Count;
